...
class ComplexToFloat(nn.Module):
    ...
    def forward(self, input):
        if not input.is_complex():
            raise ValueError(
                f"input should be a complex tensor. Got {input.dtype}")
...
class VGG(nn.Module):
    def __init__(self, vgg_name, num_classes=10):
        ...
        self.classifier = comp.ComplexConv2d(
            512*7*7, num_classes, kernel_size=1)
        # self.classifier = nn.Sequential(
